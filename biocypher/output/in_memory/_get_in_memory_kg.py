...
IN_MEMORY_DBMS = ["csv", "pandas", "tabular", "networkx", "adj_list"]
...
def get_in_memory_kg(
    ...
    if dbms in ["csv", "pandas", "tabular"]:
        return Graph(deduplicator=deduplicator, output_format="pandas")

    if dbms == "networkx":
        return Graph(deduplicator=deduplicator, output_format="networkx")
    
    if dbms == "adj_list":
        return Graph(deduplicator=deduplicator, output_format="adj_list")
