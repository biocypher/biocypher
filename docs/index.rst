...
through the Neo4j :ref:`admin-import <admin_import>` feature.
...
BioCypher using `OmniPath <https://omnipathdb.org>`_ (more specifically,
its infrastructural backend, `PyPath
<https://github.com/saezlab/pypath>`_). The adapter has the job of
...
known to the BioCypher module, we use the `schema-config.yaml
<https://github.com/saezlab/BioCypher/blob/main/config/schema_config.yaml>`_,
...
(or class) of proteins in Biolink is simply `Protein
<https://biolink.github.io/biolink-model/docs/Protein.html>`_. However,
...
<https://biolink.github.io/biolink-model/docs/PairwiseMolecularInteraction.html>`_.
...
these belong to the super-class `Associations
<https://biolink.github.io/biolink-model/docs/associations.html>`_. For
...
in the ``schema-config.yaml`` will both be ``Protein``. Again, these
should adhere to the naming scheme of Biolink.
...
:ref:`extend the Biolink schema <biolink>` in cases where there exists
no entry for our entity of choice.

Further, we are specifying the source and target classes of our
association (both ``Protein``), the label we provide in the input from
``PyPath`` (``post_translational``), and - optionally - the label we
...
represented in PascalCase and as nouns.
...
by multiple sources, e.g., `KEGG
<https://www.genome.jp/kegg/pathway.html>`_ and Reactome, which do not
...
Biolink class is the `PairwiseMolecularInteraction
<https://biolink.github.io/biolink-model/docs/PairwiseMolecularInteraction.html>`_;
...
extended version of the Biolink model
...
The BioCypher :ref:`Driver <driver>` is the main submodule of BioCypher.
It establishes a connection with a running graph database via the
:class:`neo4j.GraphDatabase.driver`, integrates the funtions of the
...
and then called on for :ref:`interacting with a running graph <running>`
and for exporting a complete database in CSV format for the :ref:`Neo4j
...
configuration specified in the :ref:`schema-config.yaml
<schema-config>`.
...
:meth:`biocypher.driver.Driver.add_nodes()` and
:meth:`biocypher.driver.Driver.add_edges()` are used to introduce new
...
`interacting with a running database <running>`_, with the data
representation being converted to a series of CSV files in a designated
output folder (standard being ``out/`` and the current datetime).
...
:meth:`biocypher.driver.Driver.write_nodes()` and
:meth:`biocypher.driver.Driver.write_edges()`. Additionally, it creates
