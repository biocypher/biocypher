...
### `directed`
- **Description:** For relationships (edges or reified as nodes), specifies whether the relationship is directed. If `true`, directionality is preserved and two edge types are created: `IS_SOURCE_OF` and `IS_TARGET_OF`. If `false` or omitted, both edges are labeled as `PART_OF`, and directionality is not preserved.
